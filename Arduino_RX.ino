  ...
  if (Serial.available() > 0) {
    ...
    if (getStarted) {                         // если приняли начальный символ (парсинг разрешён)
      if (incomingByte != '@' && incomingByte != ';') {   // если это не @ И не ;
        ...
        Data[index] = string_convert;         // кладём строку в массив
  ...
  for (byte i = 0; i < PARSE_AMOUNT; i++) {                                                         // выводим элементы массива
    ...
    if (Data[i].length() > 16) {                                                                    //если строка больше 16
      if (millis() - timer_slice[i] > 500) {
        if (x_offset[i] != 0) {                                                                     //строка "появляется" из правого края
        ...
        lcd.setCursor(0, i);
        lcd.print(Data[i].substring(slice[i], Data[i].length()) + "                ");              //выводим на дисплей, текст "изчсезает" в левом краю
